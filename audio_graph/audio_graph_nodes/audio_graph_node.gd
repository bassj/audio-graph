...
func _sample_input(input: Dictionary, time_scale: float = 1.0) -> float:
    var callable = input.get("callable", null)
    if callable:
        return callable.call(time_scale)
    ...
    return input_node.sample(output_index, time_scale)